...
const gameController = () => {
    ...
    const _updateWinner = (winner, winNum, turnDisplay) => {
        ...
        // Update winner's wins attribute
        winner.setWins(winner.getWins()+1);
        // Update winner's scoreboard
    ...
    const _newGame = (board, player1, player2, turnDisplay, playAgainButton, that) => {
        ...
        board.wipeBoard();
        let turn = [];
        // Let player 1 know it's their turn
        turnDisplay.textContent = `${player1.getName()}'s turn!`;
        /* Remove all event listeners and add an even listener for each grid cell */
        ...
        /* Add an event listener for each grid cell 
    ...
    const _playTurn = (turn, board, player1, player2, turnDisplay, that) => {
        /* Target is the grid cell that was pressed's position */
        let target = that.target.getAttribute('data-position');

        /* Remove the event listeners from the cell once it's been pressed.
        we do this by replacing the cell with a clone of itself */
        ...
        if (winStatus === 1) {
            _updateWinner(player1, 1, turnDisplay);
        } else if (winStatus === 2) {
            _updateWinner(player2, 2, turnDisplay);
    ...
    const startGame = () => {
        ...
        form.addEventListener('submit', function(event) {
            ...
            /* Generate a game board object and start a new game */
            const board = gameBoard();
            let turn = [];
            // Let player 1 know it's their turn
            turnDisplay.textContent = `${player1.getName()}'s turn!`;
            /* Add an even listener for each grid cell */
            document.querySelectorAll('.boardSquare')
                .forEach(element => element.addEventListener('click', _playTurn.bind(null, turn, board, player1, player2, turnDisplay)));
        });
